...
public class XQParser extends Lexer {
    ...
    Expression makeBinary(int op, Expression exp1, Expression exp2) throws IOException, SyntaxException {
        ...
        switch (op) {
            ...
            case 412:
                ...
                break;
            case 413:
                func = makeFunctionExp("gnu.xquery.util.ArithOp", Component.ICON_NAME_PLUS, "+");
                break;
            case 414:
            ...
            if (!(token == 10 || (token == 404 && peek() == 47))) {
                ...
                if (tokPriority >= prio) {
                    ...
                    if (token >= 422 && token <= OP_CAST_AS) {
                        ...
                        } else if (token == HttpStatus.SC_BAD_REQUEST) {
    ...
    public Expression parseFLWRInner(boolean isFor) throws IOException, SyntaxException {
        ...
        if (posDecl != null) {
            ...
            posDecl.setFlag(PlaybackStateCompat.ACTION_SET_REPEAT_MODE);
        }
        if (this.curToken == 44) {
            getRawToken();
            if (this.curToken != 36) {
