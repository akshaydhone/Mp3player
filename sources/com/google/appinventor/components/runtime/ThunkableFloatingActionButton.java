...
public class ThunkableFloatingActionButton extends AndroidNonvisibleComponent implements Component, OnOrientationChangeListener, OnClearListener {
    ...
    class C04094 implements AnimatorListener {
        C04094() {
        }
    ...
    private void create() {
        ...
        this.buttonLayout.setOnClickListener(new C04051());
    ...
    private void changeIconWithFade(final String newIconName) {
        ...
        this.iconTextView.animate().alpha(0.0f).setDuration((long) fadeDuration).setListener(new AnimatorListenerAdapter() {
            ...
            class C04061 extends AnimatorListenerAdapter {
                C04061() {
                }
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                ThunkableFloatingActionButton.this.iconTextView.animate().alpha(1.0f).setDuration((long) fadeDuration).setListener(new C04061());
            }
        }).start();
    }

    private void hideFAB() {
        this.buttonLayout.animate().scaleY(0.0f).scaleX(0.0f).setInterpolator(AnimationUtil.getFastOutSlowInInterpolator(this.context)).setDuration((long) this.animationDuration).setListener(new C04083()).start();
    }

    private void showFAB() {
        this.buttonLayout.animate().scaleX(1.0f).scaleY(1.0f).setInterpolator(AnimationUtil.getFastOutSlowInInterpolator(this.context)).setDuration((long) this.animationDuration).setListener(new C04094()).start();
