...
public final class WebViewer extends AndroidViewComponent implements ActivityResultListener, Deleteable {
    ...
    private boolean zoomDisplay = false;

    /* renamed from: com.google.appinventor.components.runtime.WebViewer$1 */
    class C04581 implements OnTouchListener {
        C04581() {
        }
    ...
    public WebViewer(ComponentContainer container) {
        ...
        this.webview = new AdvancedWebView(container.$context());
        ...
        container.$add(this);
        this.webview.setOnTouchListener(new C04581());
        this.webview.setListener(container.$context(), null, this.form.registerForActivityResult(this));
        HomeUrl("");
        Width(-2);
        Height(-2);
    }
    ...
    @SimpleProperty(category = PropertyCategory.BEHAVIOR, description = "Open link in the webviewer page using the external browser.")
    public boolean UseExternalBrowser() {
        return this.useExternalBrowser;
    }

    @DesignerProperty(defaultValue = "False", editorType = "boolean")
    @SimpleProperty(description = "Open link in the webviewer page using the external browser.")
    public void UseExternalBrowser(boolean control) {
        this.useExternalBrowser = control;
        resetWebViewClient();
    }
    ...
    @SimpleProperty(category = PropertyCategory.BEHAVIOR, description = "Determine status of the zoom control display.")
    public boolean ZoomDisplay() {
        return this.zoomDisplay;
    }

    @DesignerProperty(defaultValue = "True", editorType = "boolean")
    public void ZoomDisplay(boolean zoom) {
        this.zoomDisplay = zoom;
        this.webview.getSettings().setDisplayZoomControls(zoom);
        this.webview.getSettings().setBuiltInZoomControls(!zoom);
    }
    ...
    private void resetWebViewClient() {
        if (SdkLevel.getLevel() >= 8) {
            this.webview.setWebViewClient(FroyoUtil.getWebViewClient(this.ignoreSslErrors, this.followLinks, this.useExternalBrowser, this.container, this));
        } else {
