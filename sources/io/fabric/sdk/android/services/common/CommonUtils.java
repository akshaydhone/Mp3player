...
public class CommonUtils {
    ...
    public static final Comparator<File> FILE_MODIFIED_COMPARATOR = new C05131();
    ...
    static class C05131 implements Comparator<File> {
        C05131() {
        }
    ...
    enum Architecture {
        ...
        static Architecture getValue() {
            ...
            if (TextUtils.isEmpty(arch)) {
                Fabric.getLogger().mo1329d(Fabric.TAG, "Architecture#getValue()::Build.CPU_ABI returned null or empty");
                return UNKNOWN;
            }
...
Error: java.lang.NullPointerException
	...
	at jadx.api.JadxDecompiler$$Lambda$8/874217650.run(Unknown Source)
*/
        /*
        r3 = 0;
        r1 = 0;
    ...
    L_0x001f:
        ...
        r4.mo1332e(r5, r6, r0);	 Catch:{ Exception -> 0x001f, all -> 0x0032 }
    ...
    L_0x003c:
        ...
        r7.mo1332e(r8, r9, r2);	 Catch:{ all -> 0x005e }
        r7 = "Failed to close system file reader.";
        closeOrLog(r0, r7);
        goto L_0x003a;
    L_0x005e:
                ...
                if (!TextUtils.isEmpty(result)) {
                    ...
                    try {
                        if (result.endsWith("KB")) {
                            ...
                            Fabric.getLogger().mo1329d(Fabric.TAG, "Unexpected meminfo format while computing RAM: " + result);
                        }
                    } catch (NumberFormatException e) {
                        Fabric.getLogger().mo1332e(Fabric.TAG, "Unexpected meminfo format while computing RAM: " + result, e);
                    }
                }
    ...
    private static String hash(InputStream source, String sha1Instance) {
        try {
            ...
            Fabric.getLogger().mo1332e(Fabric.TAG, "Could not calculate hash for app icon.", e);
            return "";
        }
    }
    ...
    private static String hash(byte[] bytes, String algorithm) {
        try {
            ...
            Fabric.getLogger().mo1332e(Fabric.TAG, "Could not create hashing algorithm: " + algorithm + ", returning empty string.", e);
            return "";
        }
    }
    ...
    public static void logControlled(Context context, String msg) {
        if (isClsTrace(context)) {
            Fabric.getLogger().mo1329d(Fabric.TAG, msg);
        }
    }

    public static void logControlledError(Context context, String msg, Throwable tr) {
        if (isClsTrace(context)) {
            Fabric.getLogger().mo1331e(Fabric.TAG, msg);
        }
    }
    ...
    public static boolean getBooleanResourceValue(Context context, String key, boolean defaultValue) {
        ...
        id = getResourcesIdentifier(context, key, PropertyTypeConstants.PROPERTY_TYPE_STRING);
        if (id > 0) {
    ...
    public static String getStringsFileValue(Context context, String key) {
        int id = getResourcesIdentifier(context, key, PropertyTypeConstants.PROPERTY_TYPE_STRING);
        if (id > 0) {
    ...
    public static void closeOrLog(Closeable c, String message) {
        if (c != null) {
            try {
                ...
                Fabric.getLogger().mo1332e(Fabric.TAG, message, e);
            }
        }
    }
    ...
    public static String resolveBuildId(Context context) {
        int id = getResourcesIdentifier(context, FABRIC_BUILD_ID, PropertyTypeConstants.PROPERTY_TYPE_STRING);
        if (id == 0) {
            id = getResourcesIdentifier(context, CRASHLYTICS_BUILD_ID, PropertyTypeConstants.PROPERTY_TYPE_STRING);
        }
        if (id == 0) {
            return null;
        }
        String buildId = context.getResources().getString(id);
        Fabric.getLogger().mo1329d(Fabric.TAG, "Build ID is: " + buildId);
        return buildId;
    }
    ...
    public static void logOrThrowIllegalStateException(String logTag, String errorMsg) {
        ...
        Fabric.getLogger().mo1342w(logTag, errorMsg);
    }
